// refs. https://github.com/opentrace-community/opentrace-cloud-functions/blob/master/functions/src/opentrace/utils/CustomEncrypter.ts
...
class CustomEncrypter {
  ...
  decrypt(cipherText: string, iv: string, authTag: string) {
    ...
    // Add ts-ignore due to build error `TS2769: No overload matches this call.`
    // @ts-ignore
  ...
  encode(cipherText: string, iv: string, authTag: string): Buffer {
    const [cipherData, ivData, authTagData] = [cipherText, iv, authTag].map((e) =>
